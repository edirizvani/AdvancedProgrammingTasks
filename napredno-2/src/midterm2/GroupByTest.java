...
//        return records.stream().filter(student->student.getGrade()>5).collect(Collectors.groupingBy(Record::getStudentId,TreeMap::new,Collectors.averagingInt(Record::getGrade)));
//    }
//    Map<String, Double> coursesAverageGrade(){
//       return records.stream().filter(student->student.getGrade()>5).collect(Collectors.groupingBy(Record::getCourseName,TreeMap::new,Collectors.averagingInt(Record::getGrade)));
//    }
//    Map<String, Long> studentsPassedCoursesCount(){
//        return records.stream().filter(record->record.getGrade()>5).collect(Collectors.groupingBy(Record::getStudentId,TreeMap::new,Collectors.counting()));
//    }
//    Map<String, Long> coursesPassedStudentsCount(){
//        return records.stream().filter(record->record.getGrade()>5).collect(Collectors.groupingBy(Record::getCourseName,TreeMap::new,Collectors.counting()));
//    }
//    Map<String, List<String>> studentsPassedCourses(){
//
//        return records.stream().filter(record->record.getGrade()>5).collect(Collectors.groupingBy(Record::getStudentId,TreeMap::new,Collectors.mapping(Record::getCourseName,Collectors.toList())));
//    }
//    Map<String, Map<String, Double>> averageGradePerExamSession(){
//        return records.stream().filter(record -> record.getGrade()>5).collect(Collectors.groupingBy(Record::yearAndMonth,TreeMap::new,Collectors.groupingBy(Record::getCourseName,TreeMap::new,Collectors.averagingInt(Record::getGrade))));
//    }
//
//}
//
