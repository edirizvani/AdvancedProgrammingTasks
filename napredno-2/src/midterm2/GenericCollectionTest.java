...
public void addGenericItem(String category, T element) {
   ...
   public Collection<T> findAllBetween (LocalDateTime from, LocalDateTime to){
    ...
    public Map<String, Set<T>> byMonthAndDay(){
        return elements.values().stream().flatMap(Collection::stream)
                .collect(Collectors.groupingBy(element->element.getTimestamp().getMonth().toString()+ "-"+element.getTimestamp().getDayOfMonth(),TreeMap::new,Collectors.toCollection(()->new TreeSet<T>(Comparator.reverseOrder()))));
    }
    public Map<Integer, Long> countByYear(){
     return    elements.values().stream().flatMap(Collection::stream)
                .collect(Collectors.groupingBy(element->element.getTimestamp().getYear(),TreeMap::new,Collectors.counting()));
    }
}