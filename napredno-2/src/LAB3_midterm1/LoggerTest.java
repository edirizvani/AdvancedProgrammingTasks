...
//        LogProcessor<T> firstLogProcessor = (logs) -> logsList.stream().filter(log->log.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));
//
//        LogProcessor<T> secondLogProcessor = (logs)-> logsList.stream().filter(log->log.getMessage().length()<100).collect(Collectors.toCollection(ArrayList::new));
//
//        LogProcessor<T> thirdLogProcessor =(logs)-> logsList.stream().sorted().collect(Collectors.toCollection(ArrayList::new));
//
